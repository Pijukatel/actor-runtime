...
- **Debug mode is the one exception to "no other Actor container port is ever published"**
  (`actor-driver.md`'s "Debug mode" section): when debug mode is on for an Actor, that Actor's _own_ run
  container gets a port published on the host, bound to `127.0.0.1` (`5678` Python / `9229` Node by
  default, per-Actor overridable) - the runtime's own two ports above are unaffected, and no port is
  published for a run whose Actor never turned debug mode on.
