...
async def spa_catch_all(request: Request, full_path: str) -> Response:
    ...
    — every ``/v2/*`` API route, ``/``, ``/console`` and ``/console/app.js`` —
    matched. It serves ``index.html`` ONLY for a GET to the SPA's own top-level
    prefixes (an allowlist on the first path segment); every other unmatched
    path is a normal API 404 in the Apify envelope, so the catch-all never
    shadows the API surface.
