...
interface Input {
	...
	/** Development-only escape hatch for environments whose egress is a TLS-intercepting proxy. */
...
if (!proxyConfiguration) {
	log.warning(
		'Running without a proxy - moebelix.cz sits behind Cloudflare and will most likely block direct requests. ' +
...
interface ApolloProduct {
	...
	priceData?: {
		currentPrice?: { value?: number; currencyIso?: string } | null;
		oldPrice?: { value?: number } | null;
		priceSaving?: { savingValue?: number; savingVariant?: string } | null;
	} | null;
	mediaData?: { presentation?: { cdnFilename?: string; altText?: string }[] | null } | null;
}
 ...
 * unescaped first.
 */
function parseApolloState(body: string): Record<string, unknown> | null {
	...
	try {
		return JSON.parse(blob.replace(/\\</g, '<')) as Record<string, unknown>;
	} catch (err) {
		log.warning(`Failed to parse window.__APOLLO_STATE__: ${(err as Error).message}`);
		return null;
	}
}

/** Finds the category search result (product list + pagination) inside the Apollo state. */
function findApolloSearch(state: Record<string, unknown>): ApolloSearch | null {
	for (const entity of Object.values(state)) {
		if (!entity || typeof entity !== 'object') continue;
		for (const value of Object.values(entity as Record<string, unknown>)) {
			if (value && typeof value === 'object' && Array.isArray((value as ApolloSearch).searchResults)) {
				return value as ApolloSearch;
			}
		}
	}
	return null;
...
function productToItem(product: ApolloProduct, category: string, categoryUrl: string, page: number): ProductItem {
...
function productsFromDom($: CheerioCrawlingContext['$'], category: string, categoryUrl: string, page: number): ProductItem[] {
...
function listingPageUrl(categoryUrl: string, page: number): string {
...
const crawler = new CheerioCrawler({
	...
	async requestHandler(context) {
		...
		if (request.label === 'CATEGORY_HUB') {
			const wanted = normalize(request.userData.category as string);
			const candidates: { title: string; url: string }[] = [];
			$('[data-testid="categoryCard"] a[title][href]').each((_, el) => {
				const title = $(el).attr('title')!.trim();
				const href = $(el).attr('href')!;
				if (title && href) candidates.push({ title, url: new URL(href, BASE_URL).href });
			});
			if (candidates.length === 0) {
				throw new Error('No category cards found on the category hub page - the page may have changed or been blocked.');
			}
			const slugOf = (url: string) => normalize(new URL(url).pathname.replace(/^\//, '').replace(/-C[0-9A-Za-z]+$/, '').replace(/-/g, ' '));
			const match =
				candidates.find((c) => normalize(c.title) === wanted) ??
			...
			if (!match) {
				...
				throw new Error(
					`Category "${request.userData.category}" not found on ${CATEGORY_HUB_URL}. Available categories: ${available}`,
				);
			}
		...
		const categoryUrl = request.userData.categoryUrl as string;
		const page = request.userData.page as number;
		...
		if (page === 1 && pushedCount < maxItems && totalPages > 1) {
			...
			if (requests.length > 0) await crawlerRef.addRequests(requests);
